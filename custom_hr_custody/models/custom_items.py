...
class HrCustomCustodyItems(models.Model):
    ...
    def unlink(self):
        ...
        used_item_new = self.env['hr.custody.lines'].sudo().search([('custody_item','=',current_id)])
        _logger.info(used_item_new)
        if len(used_item_new) > 0:
            _logger.info(len(used_item_new))
