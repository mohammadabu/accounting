...
class ResPartner(models.Model):
    ...
    notes = fields.Char()
    reason_modification = fields.Many2one('hr.attendance.reason.modification', string="Reason for changing attendance")
    @api.model
        ...
        if part_master_id:
                ...
                if part_master.type == 'xlsx':
                    ...
                    #     # Sales data xlsx
                        ...
                        for rownum in range(sheet.nrows):
                            ...
                            if "اسم الموظف" in item and "التاريخ" in item and "وقت دخول" in item and "وقت الخروج" in item and "رقم الهوية" in item:
                                first_row = rownum
                        ...
                        for rownum1 in range(sheet.nrows): 
                            ...
                            if rownum1 > first_row:
                                ...
                                id_number = item_y[id_number_row] 
                                ...
                                if  full_date_check_in == False:
                                    full_date_check_in = full_date_check_out
                                if  full_date_check_in != False :
                                    try:
                                        # emp_name_info = self.env['hr.employee'].sudo().search([('name','=',emp_name)])
                                        emp_name_info = self.env['hr.employee'].sudo().search([('identification_id','=',id_number)])
                                        if emp_name_info:
                                            if full_date_check_in != False:
                                               full_date_check_in = full_date_check_in - timedelta(hours=3)
                                            if full_date_check_out != False:
                                               full_date_check_out = full_date_check_out - timedelta(hours=3)   
                                            attendance_vals = {
                                            ...
                                            list_of_failed_record += "<h1> Error at Line :" + str(rownum1 + 1) + " => Employee Not Found</h1>"
                                            _logger.error("Error at %s" % str(rownum1))            
                                    except Exception as e:
                                        # total_failed_record += 1
                                        # list_of_failed_record += "<h1> Error at Line :" + str(rownum1 + 1) + " </h1>"
                                        _logger.error("Error at %s" % str(rownum1))    
                                else:
                                    if emp_name != "":
                                        total_failed_record += 1
                                        list_of_failed_record += "<h1> Error at Line :" + str(rownum1 + 1) + " => Check - in / Not Found  </h1>"
                                        _logger.error("Error at %s" % str(rownum1))                         
            except Exception as e:
            ...
            try:
                ...
                if part_master.user_id:
                    message = "اكتملت عملية الاستيراد. تحقق في محفوظات الحضور المستوردة إذا كانت جميع الحضور" \
                              " تم استيراده بشكل صحيح. </br></br> ملف مستورد: %s </br>" \
                              "مستورد عن طريق: %s" % (
            ...
            # self.pool.get("hr.attendance").remove_finish_import_crons(self)    
