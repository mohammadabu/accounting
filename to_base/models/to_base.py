...
from odoo.tools import remove_accents, date_utils
...
class TOBase(models.AbstractModel):
    ...
    def find_first_date_of_period(self, period_name, date, start_day_offset=0):
        """
    ...
    def get_months_between_dates(self, dt_from, dt_to):
        """
        Calculate number of months (in float) between two given dates that respect odd/even months

        :return: number of months (in float) between two given dates
        :rtype: float
        """
        months = 0.0
        dt_from = fields.Datetime.to_datetime(dt_from)
        dt_to = fields.Datetime.to_datetime(dt_to)
        dates = [dt_from]
        for dt in rrule.rrule(freq=rrule.MONTHLY, dtstart=dt_from, bymonthday=1, until=dt_to):
            if dt not in dates:
                dates.append(dt)
        if dt_to not in dates:
            dates.append(dt_to)
        ...
        for idx, date in enumerate(dates):
            ...
            if idx == 1 or idx == dates_count - 1:
                months += ((date - last_seen_date).days / self.get_days_of_month_from_date(last_seen_date))
            else:
                months += 1
            last_seen_date = date
        return months
